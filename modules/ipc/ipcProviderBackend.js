...
/**
The IPC provider backend filter and tunnel all incoming request to the biblecoin node.
...
class IpcProviderBackend {
                            ...
                            return new Q((resolve, reject) => {
                                const onStateChange = (newState) => {
                                    if (ethereumNode.STATES.CONNECTED === newState) {
                                        ...
                                        log.debug(`Biblecoin node connected, resume connecting socket ${ownerId}`);
    ...
    /**
     * Handler for when Biblecoin node state changes.
     *
     * Auto-reconnect sockets when biblecoin node state changes
     *
    ...
    _onNodeStateChanged(state) {
        ...
        case ethereumNode.STATES.STOPPING:
            log.info('Biblecoin node stopping, disconnecting sockets');
