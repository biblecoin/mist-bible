...
const DEFAULT_NODE_TYPE = 'BibleCoind';
...
class EthereumNode extends EventEmitter {
    ...
    /**
     * Start an biblecoin node.
     * @param  {String} nodeType BibleCoind, eth, etc
    ...
    _start(nodeType, network, syncMode) {
        ...
        return this.stop()
            ...
            .catch((err) => {
                ...
                // if unable to start eth node then write BibleCoind to defaults
                if (nodeType === 'eth') {
                    Settings.saveUserData('node', 'BibleCoind');
                }
    ...
    __startProcess(nodeType, network, binPath, _syncMode) {
        ...
        if (nodeType === 'BibleCoind' && !syncMode) {
        ...
        return new Q((resolve, reject) => {
            ...
            this.once('data', () => {
                /*
                    ...
                    errors, etc (see BibleCoind port-binding error above)
                */
                setTimeout(() => {
    ...
    _logNodeData(data) {
        ...
        // check for BibleCoind startup errors
        if (STATES.STARTING === this.state) {
            const dataStr = data.toString().toLowerCase();
            if (nodeType === 'BibleCoind') {
                if (dataStr.indexOf('fatal: error') >= 0) {
                    const error = new Error(`biblecoind error: ${dataStr}`);
