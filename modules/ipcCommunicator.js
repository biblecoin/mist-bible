            ...
            */
            ...
            if (type === 'ethersale') {
                ...
                // BibleCoind
                    ...
                    if (process.platform === 'darwin') keystorePath += '/Library/Biblecoin/keystore';

                    if (process.platform === 'freebsd' ||
                        process.platform === 'linux' ||
                        process.platform === 'sunos') keystorePath += '/.biblecoin/keystore';

                    if (process.platform === 'win32') keystorePath = `${Settings.appDataPath}\\Biblecoin\\keystore`;
                }
...
ipc.on('backendAction_importWalletFile', (e, path, pw) => {
    ...
    const binPath = ClientBinaryManager.getClient('BibleCoind').binPath;
    const nodeProcess = spawn(binPath, ['wallet', 'import', path]);

    nodeProcess.once('error', () => {
        error = true;
        e.sender.send('uiAction_importedWalletFile', 'Couldn\'t start the "BibleCoind wallet import <file.json>" process.');
    });
...
ipc.on('mistAPI_requestAccount', (e) => {
    if (global.mode === 'wallet') {
        ...
        if (e.sender.history[0] === 'https://wallet.biblecoin.org/') {
            createAccountPopup(e);
        } else {
