...
class Settings {
    ...
    get appName() {
        return this.uiMode === 'mist' ? 'Mist' : 'Biblecoin Wallet';
    }
    ...
    get rpcIpcPath() {
        ...
        if (process.platform === 'darwin') {
            ipcPath += '/Library/Biblecoin/BibleCoind.ipc';
        } else if (process.platform === 'freebsd' ||
            process.platform === 'linux' ||
            process.platform === 'sunos') {
            ipcPath += '/.biblecoin/BibleCoind.ipc';
        } else if (process.platform === 'win32') {
            ipcPath = '\\\\.\\pipe\\BibleCoind.ipc';
        }
...
const argv = require('yargs')
    ...
    .option({
        ...
        node: {
            demand: false,
            default: null,
            describe: 'Node to use: BibleCoind, eth',
            requiresArg: true,
            nargs: 1,
            type: 'string',
            group: 'Mist options:',
        ...
        gethpath: {
            demand: false,
            describe: 'Path to biblecoind executable to use instead of default.',
            requiresArg: true,
            nargs: 1,
            type: 'string',
            group: 'Mist options:',
        ...
        syncmode: {
            demand: false,
            requiresArg: true,
            describe: 'biblecoind synchronization mode: [fast|light|full]',
            nargs: 1,
            type: 'string',
            group: 'Mist options:',
        },
        ...
        '': {
            describe: 'To pass options to the underlying node (e.g. biblecoind) use the --node- prefix, e.g. --node-datadir',
            group: 'Node options:',
        },
    })
    .help('h')
