...
let menuTempl = function (webviews) {
                ...
                submenu: [
                    {
                        ...
                        click() {
                            ...
                            // BibleCoind
                                ...
                                if (process.platform === 'darwin') {
                                    userPath += '/Library/Biblecoin/keystore';
                                }

                                if (process.platform === 'freebsd' ||
                                process.platform === 'linux' ||
                                process.platform === 'sunos') {
                                    userPath += '/.biblecoin/keystore';
                                }

                                if (process.platform === 'win32') {
                                    userPath = `${Settings.appDataPath}\\Biblecoin\\keystore`;
                                }
                            }
    ...
    if (process.platform === 'darwin' || process.platform === 'win32') {
        ...
        const gethClient = ClientBinaryManager.getClient('BibleCoind');

        if (gethClient) {
            nodeSubmenu.push({
                label: `biblecoind ${gethClient.version}`,
                checked: ethereumNode.isOwnNode && ethereumNode.isGeth,
                enabled: ethereumNode.isOwnNode,
                type: 'checkbox',
                click() {
                    restartNode('BibleCoind', null, 'fast', webviews);
                },
            });
        }
    ...
    if (ethereumNode.isOwnNode && ethereumNode.isGeth && !ethereumNode.isDevNetwork) {
        devToolsMenu.push({
            ...
            type: 'checkbox',
            click() {
                restartNode('BibleCoind', null, (ethereumNode.isLightMode) ? 'fast' : 'light');
            },
        });
    }
    ...
    helpMenu.push({
        ...
        click() {
            shell.openExternal('https://github.com/biblecoin/mist/wiki');
        },
    }, {
        label: i18n.t('mist.applicationMenu.help.gitter'),
        click() {
            shell.openExternal('https://gitter.im/biblecoin/mist');
        },
    }, {
        label: i18n.t('mist.applicationMenu.help.reportBug'),
        click() {
            shell.openExternal('https://github.com/biblecoin/mist/issues');
        },
    });
