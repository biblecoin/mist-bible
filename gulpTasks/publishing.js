...
gulp.task('upload-binaries', (cb) => {
    ...
    got(`https://api.github.com/repos/biblecoin/mist/releases?access_token=${GITHUB_TOKEN}`, { json: true })
    ...
    .then((draft) => {  // eslint-disable-line consistent-return
        if (draft.draft === true) {
            ...
            return githubUpload({
                url: `https://uploads.github.com/repos/biblecoin/mist/releases/${draft.id}/assets{?name}`,
                token: [GITHUB_TOKEN],
                assets: filePaths
            }).then((res) => {
            ...
            .then(() => {
                ...
                if (draft.body && checksums) {
                    ...
                    got.patch(`https://api.github.com/repos/biblecoin/mist/releases/${draft.id}?access_token=${GITHUB_TOKEN}`, {
                        body: JSON.stringify({
                            tag_name: `v${version}`,
