...
gulp.task('update-nodes', (cb) => {
    const clientBinariesGeth = clientBinaries.clients.biblecoind;
    const localGethVersion = clientBinariesGeth.version;
    const newJson = clientBinaries;
    const BibleCoind = newJson.clients.biblecoind;

    // Query latest BibleCoind version
    got('https://api.github.com/repos/biblecoin/go-biblecoin/releases/latest', { json: true })
    .then((response) => {
    ...
    .then((tagName) => {
        ...
        // Compare to current BibleCoind version in clientBinaries.json
        if (cmp(latestGethVersion, localGethVersion)) {
            BibleCoind.version = latestGethVersion;

            // Query commit hash (first 8 characters)
            got(`https://api.github.com/repos/biblecoin/go-biblecoin/commits/${tagName}`, { json: true })
            .then((response) => {
            ...
            .then((hash) => {
                ...
                .then((response) => {
                    ...
                    _.keys(BibleCoind.platforms).forEach((platform) => {
                        _.keys(BibleCoind.platforms[platform]).forEach((arch) => {
                            // Update URL
                            let url = BibleCoind.platforms[platform][arch].download.url;
                            url = url.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestGethVersion}-${hash}`);
                            BibleCoind.platforms[platform][arch].download.url = url;

                            // Update bin name (path in archive)
                            let bin = BibleCoind.platforms[platform][arch].download.bin;
                            bin = bin.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestGethVersion}-${hash}`);
                            BibleCoind.platforms[platform][arch].download.bin = bin;

                            // Update expected sanity-command version output
                            BibleCoind.platforms[platform][arch].commands.sanity.output[1] =
                            String(latestGethVersion);

                            // Update md5 checksum
                            blobs.forEach((blob) => {
                                if (String(blob.Name) === _.last(BibleCoind.platforms[platform][arch].download.url.split('/'))) {
                                    const sum = new Buffer(blob.Properties[0]['Content-MD5'][0], 'base64');

                                    BibleCoind.platforms[platform][arch].download.md5 = sum.toString('hex');
                                }
                            });
                        });
                    });
