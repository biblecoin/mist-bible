...
const applicationName = (options.wallet) ? 'Biblecoin Wallet' : 'Mist';
...
gulp.task('bundling-interface', (cb) => {
    ...
    if (type === 'wallet') {
        if (options.walletSource === 'local') {
            ...
            console.log(`Pulling https://github.com/biblecoin/meteor-dapp-wallet/tree/${options.walletSource} "${options.walletSource}" branch...`);
            bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/biblecoin/meteor-dapp-wallet.git \
...
gulp.task('build-dist', (cb) => {
    const appPackageJson = _.extend({}, require('../package.json'), {  // eslint-disable-line global-require
        ...
        homepage: 'https://github.com/biblecoin/mist',
        build: {
            appId: `org.biblecoin.${type}`,
            asar: true,
            directories: {
