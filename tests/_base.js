...
const gethPrivate = require('BibleCoind-private');
const Application = require('spectron').Application;
const chai = require('chai');
const http = require('http');
const ecstatic = require('ecstatic');
const express = require('express');
const ClientBinaryManager = require('biblecoin-client-binaries').Manager;
...
const startGeth = function* () {
    ...
    if (!manager.clients.biblecoind.state.available) {
        gethPath = manager.clients.biblecoind.activeCli.fullPath;
        console.info('Downloading BibleCoind...');
        const downloadedGeth = yield manager.download('biblecoind');
        gethPath = downloadedGeth.client.activeCli.fullPath;
        console.info('biblecoind downloaded at:', gethPath);
    }

    const BibleCoind = gethPrivate({
        gethPath,
        balance: 5,
        genesisBlock: {
            config: {
    ...
    console.info('biblecoind starting...');
    yield BibleCoind.start();
    console.info('biblecoind started');
...
exports.mocha = (_module, options) => {
    ...
    _module.exports[options.name || path.basename(_module.filename)] = {
        * before() {
            ...
            this.BibleCoind = yield startGeth();

            const appFileName = (options.app === 'wallet') ? 'Biblecoin Wallet' : 'Mist';
            const platformArch = `${process.platform}-${process.arch}`;
            console.info(`${appFileName} :: ${platformArch}`);

            let appPath;
            const ipcProviderPath = path.join(this.BibleCoind.dataDir, 'BibleCoind.ipc');
            ...
            this.app = new Application({
                ...
                args: [
                    '--loglevel', 'debug',
                    '--logfile', mistLogFile,
                    '--node-datadir', this.BibleCoind.dataDir,
                    '--rpc', ipcProviderPath,
                ],
        ...
        * beforeEach() {
            ...
            yield this.client.execute(() => { // Code executed in context of browser
                ...
                Tabs.upsert({_id: 'wallet'}, {$set: {
                    url: 'https://wallet.biblecoin.org',
                    redirect: 'https://wallet.biblecoin.org',
                    position: 1,
        ...
        * after() {
            ...
            if (this.BibleCoind && this.BibleCoind.isRunning) {
                console.log('Stopping BibleCoind...');
                yield this.BibleCoind.stop();
            }
...
const Utils = {
    ...
    * startMining() {
        yield this.BibleCoind.consoleExec('miner.start();');
    },
    * stopMining() {
        yield this.BibleCoind.consoleExec('miner.stop();');
    },
